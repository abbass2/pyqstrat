...
from pyqstrat.pq_utils import zero_to_nan, get_empty_np_value, infer_frequency
from pyqstrat.pq_utils import resample_trade_bars, has_display, strtup2date, assert_
...
class TradeBars:
    ...
    def __init__(self, 
                 ...
                 l: np.ndarray | None = None,  # noqa: E741: ignore # l ambiguous
                 v: np.ndarray | None = None, 
                 vwap: np.ndarray | None = None) -> None:
        '''Zeroes in o, h, l, c are set to nan'''
        assert_(len(timestamps) > 1)
        assert_(len(c) == len(timestamps))
        assert_(o is None or len(o) == len(timestamps))
        assert_(h is None or len(h) == len(timestamps))
        assert_(l is None or len(l) == len(timestamps))
        assert_(v is None or len(v) == len(timestamps))
        assert_(vwap is None or len(vwap) == len(timestamps))
    ...
    def resample(self, sampling_frequency: str) -> TradeBars | None:
        '''
    ...
    def errors(self, display: bool = True) -> pd.DataFrame | None:
    ...
    def time_distribution(self, 
                          ...
                          display: bool = True, 
                          plot: bool = True, 
                          figsize: tuple[int, int] | None = None) -> pd.DataFrame:
        '''
    ...
    def plot(self,
        ...
        data: TradeBarSeries | TimeSeries
        if self.has_ohlc():
