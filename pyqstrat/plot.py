...
class HorizontalLine:
    ...
    def __init__(self, y: float, name: str | None = None, line_type: str = 'dashed', color: str | None = None) -> None:
        self.y = y
        self.name = name
...
class VerticalLine:
    ...
    def __init__(self, x: float, name: str | None = None, line_type: str = 'dashed', color: str | None = None) -> None:
        self.x = x
        self.name = name
        self.line_type = line_type
        self.color = color
...
class DateLine:
    ...
    def __init__(self, date: np.datetime64, name: str | None = None, line_type: str = 'dashed', color: str | None = None) -> None:
        self.date = date
        self.name = name
        self.line_type = line_type
        self.color = color
...
class BucketedValues(PlotData):
    ...
    def __init__(self, name: str, 
                 ...
                 display_attributes: DisplayAttributes | None = None) -> None:
        '''
        Args:
        ...
        '''
        assert_(isinstance(bucket_names, list) and isinstance(bucket_values, list) and len(bucket_names) == len(bucket_values))
...
class TradeBarSeries(TimePlotData):
    ...
    def __init__(self, 
                 ...
                 o: np.ndarray | None,
                 h: np.ndarray | None,
                 l: np.ndarray | None,  # noqa: E741: ignore # l ambiguous
                 c: np.ndarray | None,
...
def draw_boxplot(ax: mpl.axes.Axes, 
    ...
    assert_(isinstance(values, list) and isinstance(names, list) and len(values) == len(names))
    widths = None
...
def _adjust_axis_limit(lim: tuple[float, float], values: list | np.ndarray) -> tuple[float, float]:
...
def _plot_data(ax: mpl.axes.Axes, data: PlotData) -> list[mpl.lines.Line2D] | None:
...
def draw_date_line(ax: mpl.axes.Axes, 
                   ...
                   date: np.datetime64, 
                   linestyle: str, 
                   color: str | None) -> mpl.lines.Line2D:
...
def draw_horizontal_line(ax: mpl.axes.Axes, y: float, linestyle: str, color: str | None) -> mpl.lines.Line2D:
    '''Draw horizontal line on a subplot'''
    return ax.axhline(y=y, linestyle=linestyle, color=color)


def draw_vertical_line(ax: mpl.axes.Axes, x: float, linestyle: str, color: str | None) -> mpl.lines.Line2D:
    '''Draw vertical line on a subplot'''
    return ax.axvline(x=x, linestyle=linestyle, color=color)
           

def get_date_formatter(plot_timestamps: np.ndarray, date_format: str | None) -> DateFormatter:
...
class Subplot:
    ...
    def __init__(self, 
                 data_list: PlotData | Sequence[PlotData],
                 secondary_y: Sequence[str] | None = None,
                 title: str | None = None,
                 xlabel: str | None = None,
                 ylabel: str | None = None, 
                 zlabel: str | None = None,
                 date_lines: Sequence[DateLine] | None = None,
                 horizontal_lines: Sequence[HorizontalLine] | None = None,
                 vertical_lines: Sequence[VerticalLine] | None = None,
                 xlim: tuple[float, float] | tuple[np.datetime64, np.datetime64] | None = None, 
                 ylim: tuple[float, float] | tuple[np.datetime64, np.datetime64] | None = None, 
                 height_ratio: float = 1.0,
                 display_legend: bool = True, 
                 legend_loc: str = 'best', 
                 log_y: bool = False, 
                 y_tick_format: str | None = None) -> None:
        '''
        Args:
    ...
    def _resample(self, sampling_frequency: str | None) -> None:
    ...
    def _draw(self, ax: mpl.axes.Axes, plot_timestamps: np.ndarray | None, date_formatter: DateFormatter | None) -> None:
...
class Plot:
    ...
    def _get_plot_timestamps(self) -> np.ndarray | None:
        timestamps_list = []
    ...
    def draw(self, check_data_size: bool = True) -> tuple[mpl.figure.Figure, mpl.axes.Axes] | None:
        '''Draw the subplots.
...
def _group_trades_by_reason_code(trades: Sequence[Trade]) -> dict[str, list[Trade]]:
    trade_groups: dict[str, list[Trade]] = collections.defaultdict(list)
    for trade in trades:
        reason_code = '' if trade.order.reason_code is None else ''
        trade_groups[reason_code].append(trade)
    return trade_groups


def trade_sets_by_reason_code(trades: list[Trade], 
                              marker_props: dict[str, dict] = ReasonCode.MARKER_PROPERTIES, 
