...
class MarketOrder:
    def __init__(self, symbol, timestamp, qty, reason_code = ReasonCode.NONE, status = 'open'):
        '''
        Args:
            symbol: A string
            timestamp: A numpy datetime64 indicating the time the order was placed
    ...
    def __repr__(self):
        return f'{self.symbol} {pd.Timestamp(self.timestamp).to_pydatetime():%Y-%m-%d %H:%M} qty: {self.qty}' + (
...
class LimitOrder:
    def __init__(self, symbol, timestamp, qty, limit_price, reason_code = ReasonCode.NONE, status = 'open'):
        '''
        Args:
            symbol: A string
            timestamp: A numpy datetime64 indicating the time the order was placed
    ...
    def __repr__(self):
        return f'{self.symbol} {pd.Timestamp(self.timestamp).to_pydatetime():%Y-%m-%d %H:%M} qty: {self.qty} lmt_prc: {self.limit_price}' + (
            '' if self.reason_code == ReasonCode.NONE else f' {self.reason_code}') + f' {self.status}'
...
class RollOrder:
    ...
    def __init__(self, symbol, timestamp, close_qty, reopen_qty, reason_code = ReasonCode.ROLL_FUTURE, status = 'open'):
        '''
        Args:
            symbol: A string
            timestamp: A numpy datetime64 indicating the time the order was placed
    ...
    def __repr__(self):
        return f'{self.symbol} {pd.Timestamp(self.timestamp).to_pydatetime():%Y-%m-%d %H:%M} close_qty: {self.close_qty} reopen_qty: {self.reopen_qty}' + (
            '' if self.reason_code == ReasonCode.NONE else f' {self.reason_code}') + f' {self.status}'
...
class StopLimitOrder:
    ...
    '''
    def __init__(self, symbol, timestamp, qty, trigger_price, limit_price = np.nan, reason_code = ReasonCode.NONE, status = 'open'):
        '''
        Args:
            symbol: A string
            timestamp: A numpy datetime64 indicating the time the order was placed
    ...
    def __repr__(self):
        return f'{self.symbol} {pd.Timestamp(self.timestamp).to_pydatetime():%Y-%m-%d %H:%M} qty: {self.qty} trigger_prc: {self.trigger_price} limit_prc: {self.limit_price}' + (
            '' if self.reason_code == ReasonCode.NONE else f' {self.reason_code}') + f' {self.status}'
